...
`MPERproxy` 是一个专为 MPER 代币设计的透明代理合约，遵循 ERC1967 标准。它支持通过治理机制进行逻辑合约的升级，同时代理合约本身的地址保持不变。该合约集成了 OpenZeppelin 的 `TimelockController`，以实现分阶段、有时间延迟的治理操作（已在MPSproxy中简化），增强了安全性。
