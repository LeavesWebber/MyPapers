<template>
  <el-row>
    <el-col :span="24">
      <div class="box">
        <h1 style="text-align: center">Update Committee</h1>
        <el-form
          ref="form"
          :model="form"
          status-icon
...
<script>
import { getCommitteeDetail, updateCommittee } from "../../api";
export default {
  data() {
      ...
      this.$refs[form].validate((valid) => {
        if (valid) {
          if (this.validateForm()) {
            ...
            this.form.id = Number(this.$route.query.committee_id);
            // form.id 要number类型
            ...
            updateCommittee(this.form).then((data) => {
              console.log(data);
  ...
  mounted() {
    getCommitteeDetail({ committee_id: this.committee_id }).then((res) => {
      console.log(res);
      console.log(this.ruleForm.length, "-----------length-----------");
      this.form = res.data.data;
      this.form.id = this.$route.query.committee_id;
      console.log(this.form, "form");
      // 初始化ruleForm
