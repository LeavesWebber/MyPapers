...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      console.log(row.ID);
      deleteJournalIssue({ issue_id: row.ID }).then((res) => {
        console.log(res);
        // 先弹窗提示是否删除
    ...
    getIssues() {
      ...
      // 获取用户在期刊的level
      getJournalLevel({ journal_id: this.$route.query.journal_id }).then(
        (res) => {
    ...
    submit(formData) {
      this.$refs[formData].validate((valid) => {
        if (valid) {
          this.formData.journal_id = this.$route.query.journal_id;
          // journal_id转为数字类型
          this.formData.journal_id = parseInt(this.formData.journal_id);
          this.formData.volume = parseInt(this.formData.volume);
          // 后续对表单数据的处理
          if (this.modalType === 0) {
            console.log(this.formData, "before formData");
            createJournalIssue(this.formData).then(() => {
              // 重新获取列表的接口
              this.getIssues();
            });
          } else {
            ...
            console.log(this.formData, "before formData");
            updateJournalIssue(this.formData).then(() => {
              // 重新获取列表的接口
              this.getIssues();
            });
          }
