<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="CreatedAt" label="Create Time" width="120">
      </el-table-column>
      <el-table-column fixed="right" label="Options" width="175">
        <template slot-scope="scope">
          <el-button @click="details(scope.row)" type="primary" size="mini"
            >Details</el-button
          >
          <el-button @click="popup(scope.row)" type="danger" size="mini"
            >Delete</el-button
          >
        </template>
      </el-table-column>
...
<script>
import { getCommitteeList, deleteCommittee } from "../../api";
export default {
  data() {
    return {
      tableData: [
  ...
  methods: {
    ...
    handleDelete(row) {
      console.log(row.ID);
      deleteCommittee({ committee_id: row.ID }).then((res) => {
        console.log(res);
        // 先弹窗提示是否删除
