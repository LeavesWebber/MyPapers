<template>
  <el-row>
    <el-col :span="24">
      <div class="box">
        <h1 style="text-align: center">Update Conference</h1>
        <el-form
          ref="form"
          :model="form"
          status-icon
...
<script>
import { getConferenceDetail, updateConference } from "../../api";
export default {
  data() {
      ...
      this.$refs[form].validate((valid) => {
        if (valid) {
          if (this.validateForm()) {
            ...
            this.form.id = Number(this.$route.query.conference_id);
            // form.id 要number类型
            ...
            updateConference(this.form).then((data) => {
              console.log(data);
  ...
  mounted() {
    getConferenceDetail({ conference_id: this.conference_id }).then((res) => {
      console.log(res);
      console.log(this.ruleForm.length, "-----------length-----------");
      this.form = res.data.data;
      this.form.id = this.$route.query.conference_id;
