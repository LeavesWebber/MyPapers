...
func InitAliPayClient() *alipay.Client {
	// 读取私钥文件
	privateKeyBytes, err := ioutil.ReadFile(global.MPS_CONFIG.AliPay.PrivateKey)
	if err != nil {
		global.MPS_LOG.Error("读取私钥文件失败", zap.Error(err))
		return nil
	}

	// 创建支付宝客户端
	client, err := alipay.NewClient(global.MPS_CONFIG.AliPay.AppID, string(privateKeyBytes), global.MPS_CONFIG.AliPay.IsProd)
	if err != nil {
...
func FastInstantTradePay(orderNo string, amount float64) (*response.BuyMPSWithFiatResp, error) {
	...
	if aliPayClient == nil {
		return nil, fmt.Errorf("初始化支付宝客户端失败")
	}
	// 调用支付宝交易预创建接口
	...
	if aliRsp == "" {
		return nil, fmt.Errorf("获取支付链接失败")
	}
	// 构造返回的支付响应结构体
